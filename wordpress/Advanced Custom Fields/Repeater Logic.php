...
if(have_rows($field_group)): // check if "field_group_name" has any data to show elements on a page
  while(have_rows($field_group)): the_row(); // iterate through the rows
    ...
    // ignore if statement if no post object is present in the repeater
    ...
    wp_reset_postdata(); // reset $post for the rest of the page, not necessary if no post object in repeater
